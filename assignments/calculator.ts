...
let inputChoice={    // Choice Object initialized
    control:'Y'
};
while(inputChoice.control=='y'|| inputChoice.control=='Y'){  // program will continue will input choice is other that y/Y

    let FirstNum = await inquirer.prompt([{   // get input of first number
        name: "Num1",
        type: "number",
        message: "Enter fisrt number: "}
    ]);

    let SecondNum = await inquirer.prompt([{   // get input of second number
        name: "Num2",
        type: "number",
        message: "Enter second number: "}
    ]);

    let Operator = await inquirer.prompt([{  // get operator
        name: "Option",
        type: "string",
    ...
    inputChoice = await inquirer.prompt([{  // prompt for continue y/n
        name: "control",
