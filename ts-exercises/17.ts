...
let midOfArray:number = Math.floor(GuestsList.length / 2); 
// Adding guest in the mid of array
GuestsList.splice(midOfArray,0,"Nawaz Sharif"); 
// storing current numbers of person
let j:number = GuestsList.length; 
// sending message to overspaces persons
for (let i = 0; i < j-2; i++) {   
    console.log(`Aslam-o-Alaikam Dear ${GuestsList[i]}, I can invite only two people for dinner. I am sorry I cannot invite you to the dinner.`);
    GuestsList.pop();
    }
    // sending message to lucky peoples
    for(let i:number = 0; i < GuestsList.length; i++){  
        console.log(`Aslam-o-Alaikam, Dear ${GuestsList[i]}, you are still in my invitation list and you are cordially invited for dinner in PC Hotel, Lahore on Saturday 8 PM.`);
        }
